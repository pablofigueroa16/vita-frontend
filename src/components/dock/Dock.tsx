...
export default function MegaDock() {
  ...
  return (
    <div className="fixed bottom-6 left-1/2 z-50 flex w-full -translate-x-1/2 justify-center pointer-events-none">
      <div
        className={`pointer-events-auto flex flex-col items-center gap-4 rounded-3xl border border-white/10 bg-white/10 shadow-[0_8px_35px_rgba(0,0,0,0.45)] backdrop-blur-2xl transition-all duration-300 ${
        ...
        <div className="flex items-center gap-10">
          {!open && (
            <Link href="/" className="hover:text-[#00eaff]">
              <Home size={24} className="text-white hover:text-[#00eaff] transition" />
            </Link>
          )}

          {!open && (
            <Link href="/tiendas" className="hover:text-[#00eaff]">
              <Store size={24} className="text-white hover:text-[#00eaff] transition" />
            </Link>
          )}
          ...
          <button
            ...
            className="flex h-12 w-12 items-center justify-center rounded-full border border-white/20 bg-white/20 shadow-[0_0_12px_rgba(255,255,255,0.25)] backdrop-blur-xl hover:bg-white/30 transition"
          >
            {open ? <ChevronDown size={26} className="text-white" /> : <ChevronUp size={26} className="text-white" />}
          </button>

          {!open && (
            <Link href="/live" className="hover:text-[#00eaff]">
              <Radio size={24} className="text-white hover:text-[#00eaff] transition" />
            </Link>
          )}

          {!open && (
            <Link href="/configuracion" className="hover:text-[#00eaff]">
              <Settings size={24} className="text-white hover:text-[#00eaff] transition" />
            </Link>
          )}
        </div>
      </div>
