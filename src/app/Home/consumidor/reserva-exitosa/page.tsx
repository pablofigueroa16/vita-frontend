...
export default function ReservaExitosaPage() {
  ...
  const disabledCancel = loading || !res?.id || isCanceled(res?.status);

  /* ✅ CANCELA CON RUTA COMPLETA (SIN TOKEN) */
  const onCancel = async () => {
    ...
    try {
      ...
      // si el back devuelve JSON, guardamos; si no, marcamos cancelada
      let patched: any = null;
      try {
        patched = await resp.json();
      } catch {}

      const updated: Reservation = patched?.id ? patched : { ...res, status: "CANCELED" };
        ...
        <div className={cn(GLASS_SOFT, "rounded-[30px] p-6 sm:p-8 relative overflow-hidden")}>
          ...
          <div className="relative flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
            ...
            <div className="flex gap-2 flex-wrap sm:justify-end">
              ...
              {/* ✅ BOTÓN CANCELAR (se desactiva si ya está cancelada) */}
              <button
                onClick={onCancel}
                ...
                className={cn(
                  ...
                  disabledCancel && "opacity-50 cursor-not-allowed"
                )}
                title={
                  !res?.id
        ...
        <section className={cn("mt-6 rounded-[28px] p-6 relative overflow-hidden", GLASS_CARD, HOVER_LED)}>
          ...
          {res ? (
            <div className="relative">
              ...
              {err && <p className="mt-4 text-[11px] text-red-300/90">{err}</p>}
            </div>
          ) : (
